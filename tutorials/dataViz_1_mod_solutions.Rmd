...
ggplot2::theme_set(theme_minimal()+theme(text = element_text(size=20)))
```
...
- Extract the entries corresponding to year 2002 using `filter()`.
- Assign the resulting dataframe to `gapminder_2002`.

```{r 1.2}
## extract the entries of year 2002
...
```{r 1.7}
## recreate the previous plot and set  alpha to 0.7
plot_2002 <- ggplot(data = gapminder_2002, aes(x = gdpPercap, y = lifeExp,
                                               color = continent,
                                               size =  pop))+
  geom_point(alpha = 0.7)
  # guides(size = FALSE)
  # labs(x = "GDP per Capita", y = "Life Expectancy",
  #      title = "Life Expectancy Vs. GDP per Capita (2002)")
...
- Modify `plot_2002` using `facet_wrap()` to have a separate plot for each continent. *Notice how the factes are distrubuted in rows and columns*.

```{r 2.1, fig.width=10}
## modify plot_2002 to have a separate plot for each continent
plot_2002+
...
- Modify `plot_2002` to distribute the five plots in one row.
...
ggplot(data = gapminder_2002, aes(x = log(gdpPercap), y = lifeExp,
                                  color = continent))+
  geom_point()
```
...
- Use `theme_minimal()` to change the theme of `plot_2002`. *Try to experiment with other themes!*
...
- use `ggplotly()` function to turn `plot_2002` into an interactive plot.
